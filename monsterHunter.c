...
#include <stdio.h>
#include <conio.h>
#include <stdlib.h>  // 시간으로 랜덤 만들기
#include <time.h>    // 시간 가져오기
#include <windows.h> // 콘솔창 꾸미기
#include <string.h>
...
struct _MonsterIndex monster[] = { //	몬스터 실제 정보  /  배운 거 써먹음
...
// 커서 관련 함수
CONSOLE_CURSOR_INFO Curinfo; // 커서 구조체  /  관련 사이트 : https://docs.microsoft.com/ko-kr/windows/console/console-cursor-info-str
...
{ // 딜레이 중에 입력하면 무시하는 함수 만들어봄  /  검색해서 참고함
    ...
    {             //  이 부분이 연산으로 들어가서 컴퓨터 사양마다 딜레이 값이 다를 수 있음
        Sleep(5); // windo.h에 포함된 함수  /  이 함수는 자고 있을 때도 입력을 받아버림  /  의도랑 다름
        if (_kbhit())
            while (_getch() == '\0')
                ; // 키보드 입력이 있을 때 입력값이 널값일 때 까지 반복  /  입력값을 없앤다고 보면 됨
    }
}
...
//		사용법  /  16진수로 써야함  /  앞이 배경색 뒤에가 글자색
//
//		색상		/  7. 흰색은 회색으로 보임(이 게임 배경색)  /  F. 밝은 흰색이 진짜 흰색
//		0. 검정		/	1. 파랑		/	2. 초록		/	3. 옥색
//		4. 빨강		/	5. 자주		/	6. 노랑		/	7. 흰색
//		8. 회색		/	9. 연한파랑	/	A. 연한초록	/	B. 연한옥색
//		C. 연한빨강	/	D. 연한자주	/	E. 연한노랑	/	F. 밝은흰색

void gotoxy(int x, int y)
{ // 커서 이동 부분 함수, 검색해서 가져옴	/	좌표값 지정해서 움직이게 만듬
    COORD pos = {x, y};
...
void reset(int k)
{ // 화면 초기화		/	화면 시작 줄부터 k번째 줄까지 공백으로 채움  /  바꾼다면 지정 줄부터 k까지 공백으로
    gotoxy(0, 0);
    for (int i = 0; i < k; i++)
    {
...
void Rreset(int k)
{ // 화면 역순으로초기화		/	k 줄부터 처음 줄까지 공백으로 채움  /  바꾼다면 지정 줄부터 k까지 공백으로
    gotoxy(0, 0);
    for (int i = k - 1; i > 0; i--)
    {
        textcolor(cN = resetting); // 색추가 회색
        gotoxy(0, i - 2);
        printf("                                                                                \n");
        textcolor(cN = backg); // 색추가 흰색
        gotoxy(0, i);
        printf("                                                                                \n");
        Sleep(1);
        delay(1);
...
void winText()
{
    ...
    for (int k = 0; k < 7; k++)
    {
        ...
        {
            if (winText[k][kk] == ' ')
                textcolor(cN = backg); // 공백은 배경색으로
            else
                textcolor(cN = winfont);  // 공백 아니면 winfont 색으로  /  배경 연한파랑  /  글자 흰색
...
void wdText(int result)
{
    ...
    textcolor(cN = backg); // 출력 끝나면 다시 배경색으로
